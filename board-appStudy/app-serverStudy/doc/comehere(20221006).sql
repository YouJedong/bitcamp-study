...
-- 공지사항 첨부파일
DROP TABLE IF EXISTS app_notice_file RESTRICT;

-- 보안질문
DROP TABLE IF EXISTS app_boanqna RESTRICT;
...
-- 1:1 문의
ALTER TABLE app_qa
  ADD CONSTRAINT PK_app_qa -- 1:1 문의 기본키
    PRIMARY KEY (
      qno -- 문의질문번호
    );
...
-- 공지사항 첨부파일
ALTER TABLE app_notice_file
  ADD CONSTRAINT PK_app_notice_file -- 공지사항 첨부파일 기본키
    PRIMARY KEY (
      nfno -- 첨부파일번호
    );

-- 보안질문
CREATE TABLE app_boanqna (
...
-- 보안질문
ALTER TABLE app_boanqna
  ADD CONSTRAINT PK_app_boanqna -- 보안질문 기본키
    PRIMARY KEY (
      scqno -- 보안질문번호
    );
...
-- 회원
ALTER TABLE app_member
  ADD CONSTRAINT FK_app_boanqna_TO_app_member -- 보안질문 -> 회원
    FOREIGN KEY (
      scqno -- 보안질문번호
    )
    REFERENCES app_boanqna ( -- 보안질문
      scqno -- 보안질문번호
    );
...
-- 1:1 문의
ALTER TABLE app_qa
  ADD CONSTRAINT FK_app_member_TO_app_qa -- 회원 -> 1:1 문의
    FOREIGN KEY (
      mno -- 회원번호
    )
    REFERENCES app_member ( -- 회원
...
-- 공지사항 첨부파일
ALTER TABLE app_notice_file
  ADD CONSTRAINT FK_app_notice_TO_app_notice_file -- 공지사항 -> 공지사항 첨부파일
    FOREIGN KEY (
      nno -- 게시글번호
    )

...
-- 보안질문
DROP TABLE IF EXISTS secq RESTRICT;
...
CREATE TABLE member (
  ...
  secqno   INTEGER      NOT NULL COMMENT '보안질문번호', -- 보안질문번호
...
CREATE TABLE board (
  ...
  private BOOLEAN      NOT NULL COMMENT '공개설정', -- 공개설정
...
CREATE TABLE alert (
  ...
  alcheck BOOLEAN    NOT NULL DEFAULT 1 COMMENT '읽음표시' -- 읽음표시
)
COMMENT '알림';
...
CREATE TABLE secq (
  secqno INTEGER      NOT NULL COMMENT '보안질문번호', -- 보안질문번호
  sq     VARCHAR(255) NOT NULL COMMENT '보안질문' -- 보안질문
)
COMMENT '보안질문';

-- 보안질문
ALTER TABLE secq
  ADD CONSTRAINT PK_secq -- 보안질문 기본키
    PRIMARY KEY (
      secqno -- 보안질문번호
    );

ALTER TABLE secq
  MODIFY COLUMN secqno INTEGER NOT NULL AUTO_INCREMENT COMMENT '보안질문번호';
...
-- 회원
ALTER TABLE member
  ADD CONSTRAINT FK_secq_TO_member -- 보안질문 -> 회원
    FOREIGN KEY (
      secqno -- 보안질문번호
    )
    REFERENCES secq ( -- 보안질문
      secqno -- 보안질문번호
    );

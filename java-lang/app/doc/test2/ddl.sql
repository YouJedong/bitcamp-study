...
-- 학교
DROP TABLE IF EXISTS ems_shcool RESTRICT;
...
CREATE TABLE ems_lecture (
  ...
  fdate DATE         NOT NULL COMMENT '종료일', -- 종료일
...
CREATE TABLE ems_center (
  ...
  ano      INTEGER      NOT NULL COMMENT '기본주소번호', -- 기본주소번호
  dat_addr VARCHAR(255) NOT NULL COMMENT '상세주소', -- 상세주소
  tel      VARCHAR(30)  NOT NULL COMMENT '대표전화', -- 대표전화
  fax      VARCHAR(30)  NULL     COMMENT '팩스', -- 팩스
  hoompage VARCHAR(255) NULL     COMMENT '홈페이지' -- 홈페이지
)
COMMENT '교육센터';
...
-- 학교
ALTER TABLE ems_shcool
  ADD CONSTRAINT PK_ems_shcool -- 학교 기본키
    PRIMARY KEY (
      sno -- 학교번호
    );

-- 학교 유니크 인덱스
CREATE UNIQUE INDEX UIX_ems_shcool
  ON ems_shcool ( -- 학교
    title ASC -- 학교명
  );
...
-- 회원
ALTER TABLE ems_member
  ADD CONSTRAINT FK_ems_shcool_TO_ems_member -- 학교 -> 회원
    FOREIGN KEY (
      sno -- 학교번호
    )
    REFERENCES ems_shcool ( -- 학교
      sno -- 학교번호
    );